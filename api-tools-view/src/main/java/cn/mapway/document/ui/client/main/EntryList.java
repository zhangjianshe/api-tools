    ...
    private String parseGroup(Group group, String searchText) {
        ...
        for (int i = 0; i < group.entries().length(); i++) {
            ...
            if (searchText == null || searchText.length() == 0) {
                ok = true;
            } else if (e.title().contains(searchText) || e.url().contains(searchText) || e.author().contains(searchText)) {
                ok = true;
            } else {
                ok = false;
            }
            ...
            this.setWidget(row, column++, title);
            ...
            getCellFormatter().setHorizontalAlignment(row, column - 1,
                    HasHorizontalAlignment.ALIGN_RIGHT);
            String style = row % 2 == 0 ? SysResource.INSTANCE.getCss().row0() : SysResource.INSTANCE.getCss().row1();
            rf.setStylePrimaryName(row, style);
            row++;
        }
